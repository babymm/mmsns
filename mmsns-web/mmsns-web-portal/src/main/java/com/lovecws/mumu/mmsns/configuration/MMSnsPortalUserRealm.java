...
public class MMSnsPortalUserRealm extends UserRealm {
    ...
    public AuthenticationInfo authenticationInfo(final AuthenticationToken token) {
        ...
        List<MMSnsCommonUserEntity> users = commonUserService.getCommonUserByCondition(userName, userPhone,userEmail,null);
        //用户不存在
        if (users == null || users.size() == 0 || users.size() > 1) {
            throw new UnknownAccountException();
        }
        //校验 账户状态信息
        MMSnsCommonUserEntity user = users.get(0);
        //账户被锁异常
        if (user.getUserStatus().equals(MMSnsCommonUserEntity.USER_STATUS_FORBIT)) {
            throw new DisabledAccountException();
        }
        //将用户信息保存到session中
        SecurityUtils.getSubject().getSession().setAttribute(MMSnsCommonUserEntity.MMSNS_COMMON_USER, user);
        // 交给AuthenticatingRealm使用CredentialsMatcher进行密码匹配
        SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(user.getUserName(),user.getUserPassword(),ByteSource.Util.bytes(user.getUserName() + user.getUserSalt()), getName());
